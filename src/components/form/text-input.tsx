  ...
  return (
    <div className='w-full flex flex-col gap-1.5'>
      ...
      <div className={clsx('w-full flex relative items-center', {
        ...
        <div className={`absolute left-4.5 flex text-gray-400 [&_svg:not([class*=size-])]:size-5 pointer-events-none`}>
          {leadingIcon}
        </div>
        <input
          type={localType}
          ...
          className={cn(
            ...
            w-full px-4.5 text-neutral-900 focus:ring-offset-0 
            ...
            clsx({
              ...
              'text-neutral-400 border-neutral-100': disabled
            })
            , className
          )}
        ...
        />
        {(type === 'password') ?
          <Button onClick={() => setLocalType(localType === 'password' ? 'text' : 'password')} variant={'link-gray'} className='z-1 absolute right-4.5 flex items-center text-gray-400 [&_svg]:size-4!'>
            {localType === 'password' ? <RiEyeCloseLine /> : <RiEyeLine />}
          </Button>
          : (helpIcon ? <div className={`absolute text-gray-400 right-4.5 [&_svg:not([class*=size-])]:size-4`}>
            <RiQuestionLine />
          </div> : null)}
      </div>
      {
